...
class _DetalleCompraScreenState extends State<DetalleCompraScreen> {
  ...
  Future<Map<String, dynamic>> _obtenerDatosCompraYEmpresa() async {
    ...
    final empresaId = compraData['empresaId'];
    final estado = (compraData['estado'] ?? '').toString().toLowerCase();

    // ✅ Mostrar diálogo si el estado es Rechazado
    if (estado == 'rechazado') {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        showCustomDialog(
          ...
          message:
              'Este pedido fue rechazado.\nPuedes contactar a la empresa para más información.',
          confirmButtonText: 'Entendido',
          cancelButtonText: null,
          confirmButtonColor: Theme.of(context).brightness == Brightness.dark
              ? Colors.amber
              : Colors.black,
        );
      });
    ...
    final empresaData = await obtenerEmpresa(empresaId);
  ...
  Future<Map<String, dynamic>?> obtenerEmpresa(String empresaId) async {
    try {
      ...
      Scaffold(
        ...
        body: FutureBuilder<Map<String, dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            final empresaData =
                snapshot.data!['empresa'] as Map<String, dynamic>?;
            ...
            // Creamos el mapa de datos para el PDF
            final data = {
                    ...
                    child: Row(
                      children: [
                        CircleAvatar(
                          radius: 20,
                          backgroundImage:
                              NetworkImage(empresaData['perfilEmpresa'] ?? ''),
                    ...
                    if (estado == 'Rechazado') ...[
                      Expanded(
                        child: TextButton.icon(
                          onPressed: () {
                            final empresaId = compraData['empresaId'];
                            final clienteId = currentUser?.uid;

                            navegarConSlideDerecha(
                              context,
                              ContactoEmpresaS(
                                userIdVisitante: clienteId.toString(),
                          ...
                          style: TextButton.styleFrom(
                            backgroundColor:
                                isDark ? const Color(0xFFFFAF00) : Colors.black,
                            padding: const EdgeInsets.symmetric(
